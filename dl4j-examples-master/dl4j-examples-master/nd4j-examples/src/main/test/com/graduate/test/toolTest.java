...
//        PHA.PHAProgress(channels.get(1), channels.get(2));
    ...
    public void dct_test() {
        ...
        Size daSize = new Size(newD.getRowDimension(), newD.getRowDimension());
        Mat newMat = new Mat(daSize, CV_32S);
        for (int i = 0; i < newD.getRowDimension(); i++) {
            for (int j = 0; j < newD.getRowDimension(); j++) {
                double[] array = new double[1];
                array[0] = newD.get(i,j);
                newMat.put(i, j, array);
                System.out.print(newD.get(i,j)+"   ");
            }
            System.out.println();
        }
