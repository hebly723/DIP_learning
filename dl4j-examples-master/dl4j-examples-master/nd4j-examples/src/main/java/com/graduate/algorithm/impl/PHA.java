 ...
 */
public class PHA implements Algorithm {

    /**
     * 生成hash字符串
     * @return
     */
    public byte[] hashString(Mat mat)
    {
        /**
         * 第一步，调用opencv自带函数缩小图片的尺寸
         */
        Mat thumbImage = new Mat(new Size(32, 32),CV_32S);
        resize(mat, thumbImage, new Size(32, 32), 0, 0, INTER_CUBIC);

        /**
         * 第二步，将RGB值转化为灰度
         */
        double[] pixels1 = new double[1024];
        for (int i = 0; i < 32; i++) {
            for(int j = 0; j < 32; j++) {
                pixels1[i* 32 + j] = rgbToGray(thumbImage.get(i, j));
            }
        }
         ...
         * 第三步，计算DCT矩阵
         */
        DCT_Trans dct_trans = new DCT_Trans(32);
        double[] newPixel = dct_trans.DCT(pixels1, 32);
        double[] pixels = dct_trans.cutArray(newPixel,64);
        /**
         * 第四步，计算灰度区域平均值
         */
        double avgPixel= 0;
