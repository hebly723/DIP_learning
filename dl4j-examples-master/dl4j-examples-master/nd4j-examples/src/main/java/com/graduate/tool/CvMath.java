...
public class CvMath {
     ...
     */
    public static double[][] getUR(double[][] dctMat)
    {
        int arrayLength = dctMat[0].length;
        double[][] urList = new double[arrayLength][2];
        List<Boolean> booleans = new ArrayList<>();
        for (int i = 0; i < arrayLength; i++) {
            booleans.add(true);
//            List<Double> ura = new ArrayList<>();
            double sum = 0;
            for (int j = 0; j < dctMat.length; j++)
            {
                sum += dctMat[j][i];
            }
            urList[i][0] = sum/dctMat.length;
            sum = 0;
            for (int j = 0; j < dctMat.length; j++)
            {
                sum += pow(dctMat[j][i] - urList[i][0], 2);
            }
            urList[i][1] = sqrt(sum/(dctMat.length-1));
//            urList.add(ura);
        }
        return urList;
    }
